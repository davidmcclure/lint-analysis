...
Last week I looked at some of the narratological "topics" that fall out of a hierarchical cluster of word distributions across narrative time, groups of words that tend to rise and fall together across novels. One side effect of this kind of bottom-up approach - just throwing a bunch of stuff at the wall and trying to make sense of it - is that it's easy to focus on the words that are the most semantically "focused," and, by extension, the easiest to interpret. For example, it's easy to reason about what's going on with cluster 37 (pistol, bullet, gun) or 139 (student, students, school) - which, as Ted and Scott pointed out on Twitter, might say as much about the footprint of different genres in the corpus as about "narrative," in any kind of abstract sense. But, it's harder to parse something like cluster 10, which includes, among other things, stopwords like "a," "an," "than," "these"?

[37, 139, 10 ??]

The weird thing, though, is that it turns out that these really high-frequency function words - determiners, pronouns, conjunctions, prepositions, the verb "to be," etc. - actually have very, very strong trends across narrative time - in fact, stronger than almost anything else in the dictionary. Take a look again at this graph from a couple weeks ago, which plots variance across narrative time as a function of frequency:

[variance ratios]

The X-axis shows the frequencies of the ~10k most frequent words in the corpus, and the Y-axis is the raw variance of the word's frequency across each percentile of narrative time, which gives a crude metric for the "volatility" of a word, the degree to which it isn't just a flat line. The higher the value, the more the word diverges from the uniform distribution, represented by the blue line, which plots out the amount of variance that we'd expect if everything were just random noise. The really interesting thing about this, though, is that that slope of the data is clearly steeper than the slope of this line - words seem to become *more* non-uniform as they become more frequent, even after adjusting for the expected correlation between frequency and variance. And, by a pretty large margin - on the left side of the graph, among the words that show up ~10k times, the highest-scoring words are about 2 orders of magnitude above what you'd expect; but this rises to almost 3 on the right side, for the small handful of words that show up 10-100M times.
...
This list of 100 contains all of the XX most frequent words in the dictionary. And, to get all of the 100 most frequent words, you only have to go down to position XX under this ranking.

What to make of this? It's kind of perplexing, in a sense, and not what I expected at the start. I assumed that function words would be basically flat, since I don't really think of them as having any kind of semantic "focus" that would cause them to consistently attach to any particular region across the narrative axis, in the way that things like "death" or "marriage" do. I thought they'd almost be *negative* examples of what I was looking for - words that, by virtue of their frequency, just sort of have to show up everywhere, more or less evenly. (Though I also remembered Matt Jockers' finding from Macroanalysis that the word "the" fluctuates across historical time, and a little voice in the back of my head wondered if there might be similar effects across narrative time.)
...
So, I think there really is some meaninful way in which high frequency words are the most non-uniform, the most narratologically *charged*, across the axis of the text? But, I'm not really sure what to think about that, whether my expectations were reasonable. Am I wrong to be surprised by this? Is it somehow tautologically true, is there a statistical / linguistic / narratological pressure that would make it impossible for this not to be the case, in some way?
